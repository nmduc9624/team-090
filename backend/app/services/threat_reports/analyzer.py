...
TELEMETRY_KEYWORDS = {
    ...
    "cloud_audit": ["cloud", "api key", "oauth", "consent", "iam", "bucket", "metadata", "kubernetes", "slack", "saas", "app"],
...
def analyze_report(request: ThreatReportRequest) -> HuntPackage:
    ...
    saas_oauth = _is_saas_oauth_context(title + "\n" + text)
    developer_platform = _is_developer_platform_context(title + "\n" + text)
    ...
    package = HuntPackage(
        ...
        ) if reference_is_exact else "Correlate OAuth/app activity, user login context, SaaS audit events, file export actions, destination domain/IP, and time window. Treat generated logic as a draft until validated against real SaaS/SIEM schema.",
        ...
        ) if reference_is_exact else "Escalate when the app is unapproved, has broad read/export permissions, accesses sensitive data, or the related user login/session is suspicious.",
